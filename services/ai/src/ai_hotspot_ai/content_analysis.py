...
async def analyze_content(
    ...
    *,
    ...
    text = f"{title}\n{summary}".strip()
    prompt = (
        "AI_HOTSPOT_CONTENT_ANALYSIS_V1\n"
        "你是 AI 技术资讯编辑。只依据输入内容输出一个 JSON 对象，不得补充未出现的事实。"
        "字段必须包括 titleZh, summaryZh, categoryCode, tags(最多5项), "
        "entities(对象数组，含type/name), "
        "factStatus(CONFIRMED或UNCONFIRMED), confidenceScore(0-100), recommendationReason。\n"
        f"来源：{source_name}\n来源级别：{official_level}\n标题：{title}\n正文摘要：{summary[:4000]}"
    )
    ...
    dimensions = _quality_dimensions(text, title, summary, official_level, authority_score)
    relevance = _relevance(text, tags)
    quality = round(sum(dimensions.values()) / len(dimensions), 2)
    if fact_status == "UNCONFIRMED":
        quality = max(0.0, quality - 12.0)
    final = round(relevance * 0.55 + quality * 0.45, 2)
    ...
    reason = _clean(
        str(
            ...
            or f"来自{source_name}的公开 AI 技术内容，已完成相关性、质量和事实状态检查。"
        ),
        1000,
    )
...
def _quality_dimensions(
    text: str, title: str, summary: str, level: str, authority: float
) -> dict[str, float]:
    ...
    return {
        ...
        for key, value in {
            ...
            "clarity": clarity,
            "sourceAuthority": authority_dimension,
