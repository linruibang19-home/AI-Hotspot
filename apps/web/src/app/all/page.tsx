...
import { getPublicContents, searchPublicContents } from "@/lib/public-content";
...
export default async function AllPage({ searchParams }: { searchParams: Promise<{ query?: string }> }) {
  ...
  const page = query.trim()
    ? { items: await searchPublicContents(query.trim()), nextCursor: null, hasMore: false }
    : await getPublicContents(false);
