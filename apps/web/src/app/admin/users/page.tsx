...
export default function AdminUsersPage() {
  ...
  return (
    <div className="page-shell">
      <PageHeader
        title="用户与邀请"
        description="公众用户可通过邮箱验证码注册；管理员也可创建账号或生成一次性邀请码。"
        action={
          <div className="header-actions">
            <button className="button" onClick={() => setMode("invitation")}>创建邀请码</button>
            <button className="button primary" onClick={() => setMode("user")}>创建账号</button>
          </div>
        }
      />
      ...
      {token ? (
        <div className="token-box" role="status">
          <strong>邀请码只显示这一次</strong>
          <code>{token}</code>
          <button className="button" onClick={() => void navigator.clipboard.writeText(token)}>复制链接</button>
        </div>
      ) : null}
      <section className="admin-columns">
        <article className="table-panel">
          ...
          <div className="data-table">
            <div className="data-row head"><span>用户</span><span>角色</span><span>状态</span><span>最近登录</span></div>
            {users.map((account) => (
              <div className="data-row" key={account.id}>
                <span><strong>{account.displayName}</strong><small>{account.email}</small></span>
                <span>{account.roles}</span><span>{account.status}</span>
                <span>{account.lastLoginAt ? new Date(account.lastLoginAt).toLocaleString("zh-CN") : "从未登录"}</span>
              </div>
            ))}
          </div>
        </article>
        <article className="table-panel">
          ...
          {invitations.map((invitation) => (
            <div className="invite-row" key={invitation.id}>
              <div><strong>{invitation.defaultRole}</strong><span className={`status-badge ${invitation.status.toLowerCase()}`}>{invitation.status}</span></div>
              <p>已使用 {invitation.usedCount}/{invitation.maxUses} · {new Date(invitation.expiresAt).toLocaleString("zh-CN")} 到期</p>
            </div>
          ))}
        </article>
      </section>
      {mode ? (
        <div className="dialog-backdrop">
          <section className="dialog-card compact" role="dialog" aria-modal="true">
            <div className="dialog-header">
              <h2>{mode === "user" ? "创建账号" : "创建邀请码"}</h2>
              <button className="icon-button" onClick={() => setMode(null)} aria-label="关闭">×</button>
            </div>
            {mode === "user" ? (
              <form className="form-stack" onSubmit={createUser}>
                <label>显示名称<input name="displayName" required minLength={2} /></label>
                <label>邮箱<input name="email" type="email" required /></label>
                <label>初始密码<input name="password" type="password" required minLength={12} maxLength={72} /></label>
                <label>角色<select name="role"><option>USER</option><option>EDITOR</option><option>OPERATOR</option><option>ADMIN</option></select></label>
                <button className="button primary">创建账号</button>
              </form>
            ) : (
              <form className="form-stack" onSubmit={createInvitation}>
                <label>默认角色<select name="role"><option>USER</option><option>EDITOR</option><option>OPERATOR</option></select></label>
                <label>最大使用次数<input name="maxUses" type="number" min="1" max="100" defaultValue="1" required /></label>
                <label>到期时间<input name="expiresAt" type="datetime-local" required /></label>
                <button className="button primary">生成邀请码</button>
              </form>
            )}
          </section>
        </div>
