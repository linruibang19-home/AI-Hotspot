...
> 文档状态：M6 实现基线（2026-07-18）
> 本文定义 MVP 领域关系。当前 56 张业务表、字段中文解释及 M7～M10 实现以《数据库表全景与字段中文说明》为准；迁移规则以《数据库详细设计与迁移顺序》为准。
...
erDiagram
    ...
    EMAIL_VERIFICATION_CHALLENGE }o--|| USER : verifies
    ...
    CONTENT_ITEM ||--o{ REPORT_ITEM : curated
    REPORT_ISSUE ||--o{ REPORT_SECTION : contains
    REPORT_SECTION ||--o{ REPORT_ITEM : contains
...
不得明文长期保存邀请码。公开邮箱验证码注册已在 V009 开放，邀请码不再是普通用户注册前提。
...
验证码摘要必须加入仅由服务端配置的 pepper；不保存明文。每次重新发送会使同邮箱、同用途的旧挑战失效，验证码只能成功消费一次。普通公开注册创建 ACTIVE 用户并固定分配 USER；管理员账号不通过公开注册提升角色。
