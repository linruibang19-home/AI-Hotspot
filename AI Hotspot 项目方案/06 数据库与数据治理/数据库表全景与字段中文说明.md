...
> 基准日期：2026-07-26
> 当前阶段：M10 本地交付后的信源质量、RAG 与模型连接治理专项（Flyway V001～V045）
> 数据库：PostgreSQL 16 + pgvector 0.8.5
> 本文只把已经由迁移创建的对象称为“现有表”。2026-07-26 实时核对已执行到 V045，模型连接、凭据归属和用户私有路由准备均为现有结构。
...
| `knowledge.user_provider_assignment` | 用户私有 BYOK 连接与 AI 任务的路由；以 `user_id + task_type` 为主键，保存连接、模型、超时和参数。当前为 0 行，用户入口开放前仅作为安全隔离边界 |
