...
> 基准日期：2026-07-22
> 当前阶段：M10 本地交付后的信源质量与 RAG 专项（Flyway V001～V025）
> 数据库：PostgreSQL 16 + pgvector 0.8.5
> 本文只把已经由迁移创建的对象称为“现有表”。2026-07-22 已执行到 V025，M7～M10、RAG v2 与信源质量周报对象均为现有结构。
...
当前共有 **55 张业务表**：`iam` 7 张、`source` 6 张、`content` 13 张、`governance` 4 张、`messaging` 3 张、`knowledge` 10 张、`research` 3 张、`automation` 9 张。
...
| `knowledge.chunk` | `id` 切片编号；`document_id` 文档；`ordinal` 顺序；`content_text` 切片正文；`token_count` 估算 Token；`source_url/source_published_at` 原地址/原始发布时间；`source_entity_id` 信源主体；`event_cluster_id` 事件聚类；`source_official_level` 官方等级；`authority_score/quality_score/final_score` 权威、质量、最终分；`effective_published_at` 检索采用的有效时间；`embedding vector(1024)` bge-m3 向量；`embedding_model/status` 模型/状态；`search_tsv` FTS 列；时间字段。检索无需回表即可做时间、质量、信源与事件约束。 |
...
7. 已执行的迁移绝不修改；当前到 V025，后续修复只能新增 V026+。
