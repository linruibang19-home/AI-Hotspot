...
> 基准日期：2026-07-26
> 当前阶段：M10 本地交付后的信源质量、RAG 与模型连接治理专项（Flyway V001～V046）
> 数据库：PostgreSQL 16 + pgvector 0.8.5
> 本文只把已经由迁移创建的对象称为“现有表”。2026-07-26 实时核对已执行到 V046，模型连接、凭据归属、用户私有路由、配额和回落策略均为现有结构。
...
当前共有 **59 张业务表**：`iam` 7 张、`source` 6 张、`content` 13 张、`governance` 4 张、`messaging` 3 张、`knowledge` 12 张、`research` 5 张、`automation` 9 张。数量以 `information_schema.tables` 的 `BASE TABLE` 实时核对为准。
...
### 8.1 knowledge（12 张）与 research（5 张）
...
| `knowledge.provider_connection` | 平台或用户拥有的模型厂商连接；保存协议、地址、能力、状态、烟测结果和凭据类型。数据库密钥只保存 AES-GCM 密文、随机 nonce、不可逆指纹与末四位；任何 API 均不得返回密文、nonce、指纹或原始 Key |
| `knowledge.user_provider_assignment` | 用户私有 BYOK 连接与 AI 任务的路由；以 `user_id + task_type` 为主键，保存连接、模型、超时、参数、`platform_fallback_enabled` 平台回落显式同意、`daily_query_limit` 每日查询上限和 `monthly_token_limit` 每月 Token 上限。当前测试连接已撤销，入口已开放，所有读写必须带当前用户所有权条件 |
...
| `knowledge.provider_metric` | `id` 指标；`capability/provider_name/model_name` 能力、Provider、模型；`connection_id/actor_user_id/query_run_id/credential_scope` 关联连接、使用者、RAG 查询及平台/用户凭据归属；`status/latency_ms/input_tokens/output_tokens/estimated_cost/error_code/recorded_at` 保存运行质量、用量与安全错误类别。 |
...
| `research.citation` | `id` 引用；`query_run_id` 查询；`chunk_id` 切片（可空）；`citation_no` 引用序号；`quote_text` 当时的证据快照；`retrieval_score` 召回/重排分；`support_status` 支持状态；`source_title/source_name/source_url/source_published_at` 是查询当时从真实知识库固化的不可变来源快照；`provenance_status` 为 VERIFIED 或 LEGACY_MISSING；`created_at` 创建时间。V043 后即使 Chunk 被保留策略删除，已验证来源仍可追溯；无法恢复的旧记录只标记缺失，不猜测补源。 |
...
| `research.evidence_assessment` | `citation_id` 与引用一对一关联；`claim_text` 被核验的统一主张；`evidence_stance` 为 SUPPORTS/REFUTES/UNVERIFIED 语义立场；`freshness_status` 为 CURRENT/OUTDATED/UNKNOWN 时效；`assessment_reason` 判断说明；`assessment_method` 标识模型结合规则、纯规则、默认规则或 ENTAILMENT_GUARD；`entailment_status` 为 ENTAILED/PARTIAL/UNSUPPORTED/NOT_EVALUATED；`entailment_score` 为主张信号在原文中的覆盖比例；`created_at` 创建时间。它不复用 `citation.support_status`，避免把“引用片段质量”误当成“支持或反驳立场”；UNSUPPORTED 会阻止模型把证据标记为支持。 |
...
| `automation.agent_run` | `id/agent_definition_id/user_id` 运行、定义、用户；`objective` 目标；`status/risk_level` 状态/风险；`permission_snapshot` 创建时权限快照；`retry_of_run_id/attempt_no` 重试链和尝试序号；`trace_id` 链路；`result_data/error_message/failure_code` 结果和结构化失败；`token_usage/estimated_cost` 模型用量与估算成本；`cancel_requested_at/last_heartbeat_at` 取消和恢复心跳；起止和创建时间 |
| `automation.agent_step` | `id/agent_run_id/step_no` 步骤、运行、序号；`step_type/title` 类型/标题；`status` 状态；`input_hash/result_summary` 输入指纹/结果摘要；起止时间 |
| `automation.tool_call` | `id/agent_run_id/agent_step_id` 调用关联；`tool_code/risk_level` 工具/风险；`arguments_hash/arguments` 参数指纹/参数；`result_summary/status` 结果/状态；`started_at/completed_at/created_at` 执行边界时间。90 天后参数由保留任务清空 |
...
7. 已执行的迁移绝不修改；当前到 V046，后续修复只能新增 V047+。
