...
> 基准日期：2026-07-22
> 当前阶段：M10 本地交付后的信源质量与 RAG 专项（Flyway V001～V029）
> 数据库：PostgreSQL 16 + pgvector 0.8.5
> 本文只把已经由迁移创建的对象称为“现有表”。2026-07-22 已执行到 V029，M7～M10、RAG v3.3 与信源质量周报对象均为现有结构。
...
当前共有 **56 张业务表**：`iam` 7 张、`source` 6 张、`content` 13 张、`governance` 4 张、`messaging` 3 张、`knowledge` 10 张、`research` 4 张、`automation` 9 张。
...
7. 已执行的迁移绝不修改；当前到 V029，后续修复只能新增 V030+。
