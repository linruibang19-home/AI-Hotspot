...
当前只有一个 Python `worker` 服务，共用 AI 镜像并消费采集、内容处理等业务队列。需要吞吐扩容时优先增加相同 Worker 实例；只有不同队列出现明确、持续且相互冲突的资源需求时才拆分独立服务。
...
当前 `compose.yaml` 未启用 Profile 分组，默认启动全部 10 个服务。Profile 是后续资源优化选项，不是现行运行前提。
