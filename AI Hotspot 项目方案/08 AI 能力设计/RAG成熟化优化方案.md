...
| 当前配置，有阶段计时（64/64、32、8） | 61 | 8.869s | 11.721s | 16.708s | 当前可比样本 P95 达标，仍有少量 Generation 长尾 |
...
- 完成 RAG-05：按配置代际统计、固定 12 题延迟回归、无证据集独立门禁；
- 补全 Hit@K、Precision@K、MRR、拒答准确率和结构化输出失败率；
- Prompt 抽离为可版本化模板，真正分离 System/User/Evidence；
- 为三类 Provider 增加独立超时、错误分类和一次幂等重试；
- PERF-01 中接入 OTel，先不增加独立 LLMOps 平台。
