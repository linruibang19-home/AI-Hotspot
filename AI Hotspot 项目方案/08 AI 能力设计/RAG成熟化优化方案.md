...
### RAG-07C：日历时间、宽泛周报与可信体验

2026-07-26 以产品页真实失败问题 `本周周报` 继续阶段 B。修复前系统只把“本周”转成滚动 7 天，并把“最近 7 天”交给模型自行解释，实际回答成 2026-07-13～19；证据只有一个独立信源且包含未证实内容，页面仍显示“已核验”。这不是单纯的 Prompt 问题，而是查询规划、检索多样化和产品可信表达同时存在缺口。

本轮改动和证据：

1. `temporal-v2` 按 Asia/Shanghai 计算今天、本周、本月的日历边界，查询同时使用 `windowStart/windowEnd`；Prompt 接收不可歧义的绝对日期，运行记录保存范围、意图和规划版本。
2. `WEEKLY_REPORT` 使用扩展语义查询，保留 Provider 返回的完整 32 条重排池，再执行每信源 1 条、每事件 1 条的多样化；普通事实问答继续使用原阈值和每信源 2 条，不全局放宽。
3. 最终真实调用严格覆盖 2026-07-20～26，从 IT之家、arXiv、AWS Machine Learning 取得 3 条独立证据；3 条引用、覆盖率 1.0、总延迟 8.776 秒。
4. 页面不再把“有引用”等同于“已核验”：无证据、证据有限、已交叉核验分别展示；精确时间范围、上下文/独立信源数可见，单信源、未证实、过期或覆盖不足会触发警示。
5. 历史会话保留并展示全部轮次，轮次切换同步答案、引用和评价，不再只恢复最后一轮。
6. 首次全量门禁发现当前 Prompt 最近 6 个在线样本有 1 次 JSON 结构失败。V039 的 `rag-answer-v1.1.1` 把 JSON 合同提升到 System，并在最后 Evidence 消息后再次提醒；新版真实样本结构有效。最终 58 条评测 PASS：Recall@20=1.000、nDCG@10=0.983、Hit@5=0.979、Precision@8=0.895、MRR@10=0.972、拒答准确率=1.000、引用支持率=1.000、ACL 泄漏=0。

本问题是继 RAG-07A 后第 2 条可复现真实失败，因此失败集进度为 2/30；`research.answer_feedback` 当前仍为 0 条，说明还不能用产品反馈分布决定大规模改造方向。新版 Prompt 当前也只有 1 个结构化在线样本，门禁虽然按现有规则通过，但统计置信度不足。下一轮应增加 `minimumStructuredOutputSamples`，至少积累 20～30 个固定问题样本后再判断结构失败率是否稳定低于 1%，不能把单次成功当成长期可靠。

### 接下来把 RAG 做成熟的优先顺序

1. **Claim-Evidence 校验**：把回答拆成原子主张，逐条验证引用蕴含、时间和主体一致性；覆盖率只能说明“带了引用”，不能说明“引用真的支持这句话”。
2. **真实失败集**：继续从“需改进”反馈收集 30～50 条问题，人工补齐期望证据、失败分类和严重级别；未经标注不得自动进入 TEST/CANARY。
3. **结构感知 Chunk / parent-child**：优先选择长文、列表和跨段落问题做单变量 A/B，比较 Recall、Precision、引用贴合、Token 和 P95，不直接全量重切。
4. **意图化检索策略**：宽泛综述、比较、事实、官方限定和无证据问题分别配置最小候选、来源配额、上下文预算与拒答阈值，避免一个全局阈值服务所有问题。
5. **真实流式体验**：实现可取消的 SSE/流式生成并记录 TTFT；先展示已完成检索的证据摘要，再增量显示回答，不能用前端假进度冒充流式。
6. **可观测性与成本**：Linux Beta 时接入 OpenTelemetry + Prometheus/Grafana，串联查询规划、SQL、Embedding、Rerank、Generation、Token、成本和降级；现阶段仍不需要更换 PostgreSQL/pgvector，也不需要 LangGraph、Kafka 或 Elasticsearch。
