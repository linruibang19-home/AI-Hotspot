...
> 文档状态：历史阶段规划。M0～M10 已完成本地交付；当前状态与下一步只看《项目进度与剩余任务》和《后续任务清单》，不要继续沿本文的旧“下一阶段”描述执行。
...
7. X 当前不进入采集实现，仅保留不可启用的未来 Connector 契约；微信公众号不进入任何 MVP 里程碑。
...
- 公众可以通过邮箱验证码注册和登录，管理端仍只允许管理员账号访问；
- 游客只能访问公开页面；
- USER 不能修改信源；
...
> 实施状态：已于 2026-07-17 完成迁移、真实信源、故障恢复、权限隔离与浏览器验收；详细证据见《M3RSS垂直闭环验收记录》。
...
> 实施状态：已于 2026-07-17 完成代码、迁移、真实信源、数据隔离、故障治理与自动化验收；详细证据见《M4公开Connector与正式信源目录验收记录》。
...
- 失败分类明确；
- 入口级限速和响应大小限制有效；
- X 仅为 RESERVED 契约且不可启用、不可派发；不存在 X 或微信公众号 Collector；
- 采集任务可追踪到 MinIO 原件。

## 8. M5：内容质量、准入和事件

> 实施状态：已于 2026-07-18 完成迁移、结构化 AI、评分、去重、事件、审核治理、Docker 与浏览器验收；详细证据见《M5内容质量事件与审核治理验收记录》。
